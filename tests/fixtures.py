...
class FictionalJobFixture:
    def make_job(self,major="计算机科学与技术",when=None,completeness="full_detail",source_kind="official_ats",deadline_days=15,status_text="立即申请",source_when=None):
        ...
        lines=[self.company,"2027校园招聘",self.title,"编号：TEST-001","招聘类型：校园招聘","地点：上海；北京","学历：硕士","专业："+major,"毕业届别：2027届",
               "职责：推荐系统建模与数据分析","技能：Python、机器学习","工具：Python","经验：应届生","截止："+last,"状态："+status_text,"投递链接："+url+"/apply"]
        text="\n".join(lines)
        ...
        facts={"location_raw":fact("地点：上海；北京"),"recruitment_type_raw":fact("招聘类型：校园招聘"),"duties_raw":fact("职责：推荐系统建模与数据分析"),
        ...
        job={"employer_key":"test_employer","company":self.company,"source_id":"test_source","job_id":"TEST-001","campaign":"2027校园招聘","title":self.title,
            "locations":["上海"],"job_type":"campus","industry":"人工智能","match_track":"ai_data","url":url,"application_url":url+"/apply","application_evidence":ref("投递链接："+url+"/apply"),"observed_at":captured,
            ...
            "match_reason":"测试推断：计算机与AI交叉；日期仅到日按+08:00日末约定。","matched_skills":["Python","机器学习"]}
